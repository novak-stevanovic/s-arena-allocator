...
/* SArena is a simple arena allocator. It contains regions organized into a
 * region list. When the tail of the region list runs out of memory in its
 * internal memory pool, a new region is pushed back to the list.
 *
...
SArena* s_arena_create(size_t region_cap, sa_err* out_err);
...
void s_arena_destroy(SArena* arena);
...
void* s_arena_alloc(SArena* arena, size_t size, sa_err* out_err);
...
void s_arena_rewind(SArena* arena);
